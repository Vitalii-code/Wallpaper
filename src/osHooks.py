...
def set_wallpaper(path: str):
    if platform.system() == "Windows":
        ...
        if os.environ.get("GNOME_DESKTOP_SESSION_ID"):
            os.system(
                f'gsettings set org.gnome.desktop.background picture-uri "{path}"'
            )
            os.system(
                f'gsettings set org.gnome.desktop.background picture-uri-dark "{path}"'
            )
            ...
            os.system(
                f'xfconf-query -c xfce4-desktop -p  /backdrop/screen0/monitor0/workspace0/last-image -s "{path}"'
            )
    ...
    if platform.system() == "Windows":
        ...
        if os.environ.get("GNOME_DESKTOP_SESSION_ID"):
            name = subprocess.check_output(
                ["gsettings get org.gnome.desktop.background picture-uri"], shell=True
            )
            ...
            name = subprocess.check_output(
                [
                    """kreadconfig5 --file "$HOME/.config/plasma-org.kde.plasma.desktop-appletsrc" --group "Containments" --group "1" --group "Wallpaper" --group "org.kde.image" --group "General" --key "Image" """
                ],
                shell=True,
            )
            return decode_string_from_terminal(name)
